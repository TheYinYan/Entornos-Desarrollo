# GitHub issues (por parejas)

[Readme Principal](/README.md) <br>
[Readme T1.1](../README.md)

1. Creamos el HTML

    ![Imagen](../Imagenes/Tarea9/1.png)

2. Creamos un issues

    ![Imagen](../Imagenes/Tarea9/2.png)

3. Creamos otro y etiquetmos con lo etiqueta documentación 

    ![Imagen](../Imagenes/Tarea9/3.png)

4. Subimos  la correciones 

    ![Imagen](../Imagenes/Tarea9/5.png)

5. Cerramos el issues

    ![Imagen](../Imagenes/Tarea9/4.png)
    ![Imagen](../Imagenes/Tarea9/6.png)



[Readme Principal](/Readme.md) <br>
[Readme T1.1](../README.md)